	...
	BossState_Idle_Param.Stay = [=](float _Delta, class GameEngineState* _Parent)
		{
			...
			{
				int a = 0;
				FSM_BossState.ChangeState(FSM_BossState::TeleportInGround);
				return;
			}
...
#pragma endregion
			...
			if (WallJumpTimer > 0.8f && false == IsEndJumpAttack)
			{
				...
				if (BulletCurTime > 0.01f)
				{
					...
					BossNewBullet->SetDirection(DIrectionVector, Angle);
...
void Boss::FSM_Boss_TeleportInGround()
{
	CreateStateParameter BossState_TeleportInGround_Param;

	BossState_TeleportInGround_Param.Start = [=](class GameEngineState* _Parent)
		{

			// 보스 캐릭터가 이동할 위치를 랜덤하게 지정해줍니다.
			GameEngineRandom Random;

			// 패턴 랜덤 선택
			int Count = Random.RandomInt(0, 1);
			switch (Count)
			{
				// 오른쪽
			case 0:
				Transform.SetLocalPosition({ 1080.0f, -510.0f });
				break;

				// 왼쪽
			case 1:
				Transform.SetLocalPosition({ 200.0f, -510.0f });
				break;
			...
			// 플레이어의 위치를 기준으로 자신의 방향을 결정한 다음
			DirCheck();

			// 애니메이션을 변경해줍니다.
			BossMainRenderer->ChangeAnimation("TeleportInGround");
		};
	...
	FSM_BossState.CreateState(FSM_BossState::TeleportInGround, BossState_TeleportInGround_Param);
}

void Boss::FSM_Boss_TeleportOutGround()
{
	CreateStateParameter BossState_TeleportOutGround_Param;

	BossState_TeleportOutGround_Param.Start = [=](class GameEngineState* _Parent)
		{
			// 애니메이션을 변경해줍니다.
			BossMainRenderer->ChangeAnimation("TeleportOutGround");
		};
	...
	FSM_BossState.CreateState(FSM_BossState::TeleportOutGround, BossState_TeleportOutGround_Param);
}
